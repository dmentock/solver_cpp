...
void DiscretizationGrid::init(bool restart) {
  ...
  double origin[3];
  Tensor<int, 1> materialAt_global;
  if (world_rank == 0) {
  ...
  Eigen::array<int, 3> local_grid = {cells[0], cells[1], cells2};
  size2 = geom_size[2] * cells2 / cells[2];
  size2Offset = geom_size[2] * cells2_offset / cells[2];
  Eigen::array<double, 3> local_size = {geom_size[0], geom_size[1], size2};
  ...
  Tensor<int, 1> materialAt(n_materialpoints_local);
...
void DiscretizationGrid::calculate_ipCoordinates0(Tensor<double, 2>& ipCoordinates0, 
...
void DiscretizationGrid::calculate_nodes0(Tensor<double, 2>& nodes0, 
                                          std::array<int, 3>& cells, 
                                          std::array<double, 3>& geom_size, 
                                          int cells2_offset) {
