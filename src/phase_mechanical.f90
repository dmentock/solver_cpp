...
function integrateStress(F,Fp0,Fi0,Delta_t,ph,en) result(broken)
  ...
  LiLoop: do
    ...
    LpLoop: do
      ...
      ! print *, "math_I3", math_I3
      ! print *, "Delta_tF", Delta_t
      ...
      ! print *, "YOOYOYO", NiterationStressLp
      ! print *, "dS_dFe", dS_dFe
      ! print *, "dS_dFi", dS_dFi
      ! print *, "Fe", Fe
      ! print *, "Fi_new", Fi_new
      ...
      ! print *, "Fe", Fe
      ! print *, "Fi_new", Fi_new
      call plastic_LpAndItsTangents(Lp_constitutive, dLp_dS, dLp_dFi, S, Fi_new, ph,en)
...
!  prepare for integration
    if (todo) then
      ...
      ! print *, "F0", F0
      ! print *, "step", step
      ! print *, "phase_mechanical_F(ph)%data(1:3,1:3,en)", phase_mechanical_F(ph)%data(1:3,1:3,en)
      ! print *, "phase_mechanical_F0(ph)%data(1:3,1:3,en)", phase_mechanical_F0(ph)%data(1:3,1:3,en)
      ! stop 36363
      F = F0 &
...
module function phase_P(co,ce) result(P)
  ...
  ! print *, "material_ID_phase", material_ID_phase
...
module subroutine phase_set_F(F,co,ce)
  ...
  ! print *, "phase_mechanical_F(material_ID_phase(co,ce))%data(1:3,1:3,material_entry_phase(co,ce))", phase_mechanical_F(material_ID_phase(co,ce))%data(1:3,1:3,material_entry_phase(co,ce))
  ! print *, "F", F
