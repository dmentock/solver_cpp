...
!--------------------------------------------------------------------------------------------------
subroutine CLI_init(material_path, load_path, grid_path, numerics_path)
...
--  UNSUPPORTED PETSc VERSION --- UNSUPPORTED PETSc VERSION --- UNSUPPORTED PETSc VERSION ---
 ...
 ! http://patorjk.com/software/taag/#p=display&f=Lean&t=DAMASK%203
  ...
  if (present(material_path)) then
    materialArg = material_path
  end if
  if (present(load_path)) then
    loadArg = load_path
  end if
  if (present(grid_path)) then
    geomArg = grid_path
  end if
  if (present(numerics_path)) then
    numericsArg = numerics_path
  end if
